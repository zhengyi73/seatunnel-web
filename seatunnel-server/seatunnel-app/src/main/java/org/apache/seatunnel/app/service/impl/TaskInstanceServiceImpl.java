            ...
            if (jobMode != null) {
                ...
                for (SeaTunnelJobInstanceDto record : records) {
                    if (record.getId() != null
                            && record.getJobEngineId() != null
                            && record.getJobType() != null) {
                        groupedByType
                                .computeIfAbsent(record.getJobType(), k -> new ArrayList<>())
                                .add(record);
                    }
                }
                for (Map.Entry<JobMode, List<SeaTunnelJobInstanceDto>> entry :
                    ...
                    Map<Long, JobSummaryMetricsRes> groupResult =
                            jobMetricsService.getALLJobSummaryMetrics(
